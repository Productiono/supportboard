    ...
    */
    ...
    var SBReports = {
        ...
        open: function (category_name) {
            ...
            setTimeout(() => {
                reports_area.find('#' + category_name).click();
            }, 300);
        },
    ...
    var SBProfile = {
        ...
        populate: function (user, profile_area, is_extra = true) {
            ...
            if (user.isExtraEmpty() && is_extra) {
                ...
                profile_area.html(`<div class="sb-profile-list"><ul class="ul-user">${code[0]}</ul>${code[1] ? `<div class="sb-title">${sb_('Conversation')}</div><ul class="ul-conversation">${code[1]}</ul>` : ''}${code[2] ? `<div class="sb-title">${sb_('Technology')}</div><ul class="ul-technology">${code[2]}</ul></div>` : ''}`);
    ...
    */
    ...
    var SBAdmin = {
        ...
        infoPanel: function (text, type = 'info', onConfirm = false, id = '', title = '', scroll = false, skip = false, onCancel = false) {
            ...
            box.find('.sb-title').html(title.includes('icon-') ? `<i class="sb-${title}"></i>` : title ? sb_(title) : (type == 'alert' ? '<i class="sb-icon-help"></i>' : ''));
                    ...
                    if (reports_area.sbLoading()) {
                        SBSettings.loadDatePicker(() => {
                            $.getScript(SB_URL + '/vendor/chart.min.js', () => {
                                reports_area.find('#sb-date-picker').daterangepicker(SBSettings.getDatePickerSettings(false, false, true)).val('');
