...
function sb_updates_validation() {
    if (1 || sb_isset($_COOKIE, 'sb-updates') != SB_VERSION && !sb_is_debug()) {//temp
        sb_cloud_load();
        $save = false;
        $settings = false;
        try {
...
function sb_updates_available() {
    ...
    foreach ($installed_apps_versions as $key => $value) {
        if ($value && isset($latest_versions[$key]) && $value != $latest_versions[$key]) {
            return true;
        }
    }
    return false;
...
function sb_utc_offset($is_timezone = false) {
    global $SB_UTC_OFFSET_2;
    if (is_numeric($SB_UTC_OFFSET_2)) {
        return $SB_UTC_OFFSET_2;
    }
    $timezone = explode('|', sb_get_setting('timetable-utc', 0));
    $SB_UTC_OFFSET_2 = sb_isset($timezone, $is_timezone ? 1 : 0, $is_timezone ? '' : 0);
    if (!$is_timezone && isset($timezone[1])) {
        $timezone = new DateTimeZone($timezone[1]);
        $now = new DateTime('now', $timezone);
        $current_offset_hours = -($timezone->getOffset($now) / 3600);
        if ($current_offset_hours !== $SB_UTC_OFFSET_2) {
            $difference = $current_offset_hours - $SB_UTC_OFFSET_2;
            $SB_UTC_OFFSET_2 += $difference;
        }
    }
    return $SB_UTC_OFFSET_2;
}
    ...
    if ($active_user) {
        ...
        switch ($automation['type']) {
            case 'messages':
                ...
                break;
            case 'emails':
                $response = empty($active_user['email']) ? false : sb_email_send($active_user['email'], sb_merge_fields(sb_t($automation['subject'])), sb_merge_fields(sb_email_default_parts(sb_t($automation['message']), $active_user_id)));
